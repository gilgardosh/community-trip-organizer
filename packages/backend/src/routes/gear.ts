...
router.get('/trip/:tripId', gearController.getGearItemsByTripId);

// Get gear summary for a trip
// All authenticated users can view (based on role and trip status)
router.get('/trip/:tripId/summary', gearController.getGearSummary);
...
router.post('/:id/assign', gearController.assignGear);

// Remove gear assignment from a family
// Families can remove their own assignments, trip admins can remove any
router.delete('/:id/assign/:familyId', gearController.removeGearAssignment);
