...
router.get('/all', protect, (req, res) => {
  res.status(200).json({ message: 'All authenticated users can access this route' });
});
...
router.get('/admin', protect, authorize(Role.TRIP_ADMIN, Role.SUPER_ADMIN), (req, res) => {
  res.status(200).json({ message: 'Only TRIP_ADMIN and SUPER_ADMIN users can access this route' });
});

// Route accessible only by SUPER_ADMIN role
router.get('/super-admin', protect, authorize(Role.SUPER_ADMIN), (req, res) => {
  res.status(200).json({ message: 'Only SUPER_ADMIN users can access this route' });
});
