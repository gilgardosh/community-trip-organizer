...
router.post('/', rateLimiters.auth, familyController.createFamily, invalidateCache(/^GET:.*\/api\/families$/));
...
router.get('/', cacheResponse({ ttl: 300 }), rateLimiters.api, familyController.getAllFamilies);

// Get family by ID
// SUPER_ADMIN, TRIP_ADMIN: can view any family
// FAMILY: can only view their own family
router.get('/:id', cacheResponse({ ttl: 600 }), rateLimiters.api, familyController.getFamilyById);
...
router.get('/:id/members', cacheResponse({ ttl: 300 }), rateLimiters.api, familyController.getFamilyMembers);

// Get adults of a family
router.get('/:id/adults', cacheResponse({ ttl: 300 }), rateLimiters.api, familyController.getFamilyAdults);

// Get children of a family
router.get('/:id/children', cacheResponse({ ttl: 300 }), rateLimiters.api, familyController.getFamilyChildren);
