...
passport.use(
  new FacebookStrategy(
    ...
    (accessToken: string, refreshToken: string, profile: FacebookProfile, done: VerifyCallback) => {
      // Wrap in void to ignore Promise
...
async function handleOAuthUser(
  ...
  try {
    ...
    done(error instanceof Error ? error : new Error('Unknown error during OAuth authentication'));
    return;
  }
}
