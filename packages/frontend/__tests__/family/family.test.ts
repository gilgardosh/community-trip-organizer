...
import { createFamilySchema, updateFamilySchema, addMemberSchema, updateMemberSchema } from '@/lib/validation'
...
describe('Family Validation Schemas', () => {
  ...
  describe('Hebrew Error Messages', () => {
    ...
    it('should return Hebrew error messages for weak password', () => {
      ...
      if (!result.success) {
        const errors = result.error.issues
        const passwordError = errors.find((e: any) => e.path.includes('password'))
