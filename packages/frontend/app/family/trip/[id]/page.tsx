...
import { getTripWithParticipation, getGearItemsByTripId, updateFamilyParticipation } from "@/lib/api"
...
export default function TripDetailsPage() {
  ...
  const [participation, setParticipation] = useState<FamilyParticipation | null>(null)
  const [gearItems, setGearItems] = useState<GearItem[]>([])
  const [loading, setLoading] = useState(true)
  const [isAttending, setIsAttending] = useState(false)
  const [dietaryRequirements, setDietaryRequirements] = useState("")
  const [gearCommitments, setGearCommitments] = useState<Record<string, number>>({})
  ...
  useEffect(() => {
    const fetchTripData = async () => {
      try {
        ...
        const { trip: tripData, participation: participationData } = await getTripWithParticipation(
          params.id as string,
          familyId,
        )
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <Card>
          <CardContent className="text-center py-12">
            ...
            <p className="text-muted-foreground mb-4">הטיול המבוקש לא קיים במערכת</p>
        ...
        <div className="bg-card border-b border-border">
          <div className="max-w-4xl mx-auto px-4 py-6">
            ...
            <div className="space-y-4">
              ...
              {trip.description && <p className="text-foreground leading-relaxed">{trip.description}</p>}
            </div>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          <Card>
            <CardContent className="text-center py-12">
              ...
              <h2 className="text-xl font-semibold mb-2">המשפחה לא משתתפת בטיול זה</h2>
              <p className="text-muted-foreground">
                המשפחה שלכם לא רשומה כמשתתפת בטיול זה. לפרטים נוספים פנו למנהל הטיול.
              </p>
            </CardContent>
          </Card>
        </div>
  ...
  const isAttendanceCutoffPassed = trip.attendanceCutoff ? new Date() > new Date(trip.attendanceCutoff) : false
          ...
          <CardContent className="space-y-4">
            {isAttendanceCutoffPassed && trip.attendanceCutoff && (
              <Alert>
                ...
                <AlertDescription>
                  תאריך הרשמה אחרון עבר ({new Date(trip.attendanceCutoff).toLocaleDateString("he-IL")}). לא ניתן לשנות
                  את סטטוס ההשתתפות.
                </AlertDescription>
              </Alert>
            )}
          ...
          <>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>אנא ציינו אלרגיות, העדפות תזונתיות או דרישות מיוחדות עבור בני המשפחה</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <Button onClick={handleSaveDietaryRequirements} className="w-full sm:w-auto">
                  <CheckCircle className="w-4 h-4 ml-2" />
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>עזרו לקהילה על ידי התנדבות להביא ציוד משותף לטיול</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {gearItems.map((item) => (
                      ...
                      {item.quantityAssigned < item.quantityNeeded && (
                        <div className="flex items-center gap-2">
                          <Input
                            ...
                            onChange={(e) => handleGearCommitment(item.id, Number.parseInt(e.target.value) || 0)}
                          />
                          <Button
                            variant="secondary"
                            size="sm"
                            disabled={!gearCommitments[item.id] || gearCommitments[item.id] <= 0}
                            onClick={async () => {
                              try {
                                await updateFamilyParticipation(params.id as string, "family1", {
                                  gearCommitments: {
                                ...
                                alert(`התחייבתם להביא ${gearCommitments[item.id]} יחידות של ${item.name}`)
                                setGearCommitments((prev) => ({ ...prev, [item.id]: 0 }))
                              } catch (error) {
                                alert("שגיאה בשמירת התחייבות הציוד")
                              }
                            }}
                          >
                            המשפחה שלנו תביא {gearCommitments[item.id] || 0} יחידות
                          </Button>
                        </div>
                      )}
                    </div>
        ...
        {trip.photoAlbumUrl && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>צפו בתמונות מהטיול ושתפו את התמונות שלכם</CardDescription>
            </CardHeader>
            <CardContent>
              <Button
                variant="secondary"
