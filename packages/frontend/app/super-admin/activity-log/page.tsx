...
import { Search, Filter, ChevronLeft, ChevronRight, Activity, User, Calendar, Database } from "lucide-react"
...
export default function ActivityLogPage() {
  ...
  const getActionBadge = (action: string) => {
    switch (action) {
      case "create":
        return <Badge className="bg-secondary text-secondary-foreground">יצירה</Badge>
      case "update":
        return (
  ...
  const filteredLog = activityLog.filter((entry) => {
    ...
    const matchesEntityFilter = entityFilter === "all" || entry.entityType === entityFilter
    const matchesActionFilter = actionFilter === "all" || entry.action === actionFilter
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground">מעקב אחר פעולות משתמשים ושינויים במערכת</p>
          </div>
                    ...
                    <TableRow key={entry.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <div className="text-right">
                            <div className="font-medium text-sm">{entry.entityName}</div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div className="text-sm text-muted-foreground">
                מציג {startIndex + 1}-{Math.min(startIndex + itemsPerPage, filteredLog.length)} מתוך{" "}
